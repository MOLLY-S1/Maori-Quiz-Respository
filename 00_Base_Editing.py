...
Questions("Numbers", 'Tahi', 'One',
          "num_list")
Questions("Numbers", 'Rua', 'Two',
          "num_list")
Questions("Numbers", "Toru", 'Three',
          "num_list")
Questions("Numbers", 'Whā', "Four",
          "num_list")
Questions("Numbers", 'Rima', "Five",
          "num_list")
Questions("Numbers", "Ono", "Six",
          "num_list")
Questions("Numbers", 'Whitu', "Seven",
          "num_list")
Questions("Numbers", 'Waru', "Eight",
          "num_list")
Questions("Numbers", 'Iwa', "Nine",
          "num_list")
Questions("Numbers", "Tekau", "Ten",
          "num_list")

Questions("Colours", 'Mā', 'White',
          "colour_list")
Questions("Colours", 'Whero', 'Red',
          "colour_list")
Questions("Colours", "Karaka", 'Orange',
          "colour_list")
Questions("Colours", 'Kōwhai', "Yellow",
          "colour_list")
Questions("Colours", 'Kākāriki', "Green",
          "colour_list")
Questions("Colours", "Mangu", "Black",
          "colour_list")
Questions("Colours", 'Kikorangi', "Blue",
          "colour_list")
Questions("Colours", 'Waiporoporo', "Purple",
          "colour_list")
Questions("Colours", 'Parauri', "Brown",
          "colour_list")
Questions("Colours", "Kiwikiwi", "Grey",
          "colour_list")
...
def check(entry, window):
    ...
    if enter == "":
        ...
        Label(error_screen, text="That was not a valid input, "
                                 "please enter your name").pack(side=TOP)
        (Button(error_screen, text="OK", command=error_screen.destroy).
         pack(side=TOP))
    else:
        window.destroy()
...
def quiz():
    ...
    def play():
        ...
        if len(asked_questions[subject]) == len(questions[subject]):
            ...
            Button(new_window, text="Exit",
                   command=lambda: check(name_entry, new_window)).pack()
            return
        ...
        question = random.choice([key for key in questions[subject] if
                                  key not in asked_questions[subject]])
        asked_questions[subject].append(question)
        ...
        while n1 == questions[subject][question]:
            n1 = random.choice(qlist)

        n2 = random.choice(qlist)
        while n2 == questions[subject][question] or n2 == n1:
            n2 = random.choice(qlist)

        answers = [n1, n2, questions[subject][question]]
        ...
        def buttons(clicked_answer):
            ...
            if clicked_answer == questions[subject][question]:
                ...
                Label(answer_window,
                      text=f"INCORRECT\n The answer was "
                           f"{questions[subject][question]}",
                      fg="red").pack(side=TOP)
            Button(answer_window, text="Next Question",
                   command=lambda: [answer_window.destroy(), play()]).pack(
                side=BOTTOM)

        # Question Screen
        Label(new_window,
              text=f"What is the English word for {question}?").pack(side=TOP)
        # Make a button for each possible answer
        for answer in answers:
            Button(new_window, text=f"{answer}",
                   command=lambda ans=answer: buttons(ans)).pack(side=TOP)
...
def instructions():
    ...
    Label(instructions_window,
          text="INSTRUCTIONS:\n"
               "When you press 'Start Quiz' you will be given "
               "two options for quiz type.\n"
               "Once you choose a quiz type, there will be 10 "
               "questions on the subject of your choice. "
               ...
               "in the home screen to see your name on the "
               "leaderboard!").pack(side=TOP)
    Button(instructions_window, text="Start Quiz", command=start_quiz).pack()
    Button(instructions_window, text="Return to Menu",
           command=lambda: instructions_window.destroy()).pack()
    root.mainloop()
...
def statistics():
    ...
    def scoring():
        ...
        # Sorting scoreboard
        sorted_scores = sorted(scoreboard.items(),
                               key=lambda x: int(x[1].split('/')[0]),
                               reverse=True)
        scoreboard = dict(sorted_scores)

        Label(new_window, text="SCOREBOARD", fg="Black",
        ...
        for key, value in sorted_scores:
            ...
            place += 1
        Button(new_window, text="Exit",
               command=lambda: new_window.destroy()).pack(side=TOP)
        root.mainloop()
...
def welcome_screen():
    # Welcome Banner
    root.title("WELCOME")
    welcome = Label(root, bg="White", fg="Red",
                    text="Welcome to the Māori Quiz",
                    font=("Times", 20, "bold"))
