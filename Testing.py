...
def play():
    ...
    if len(asked_questions[enter]) == len(questions[enter]):
        ...
        def exit_play():
            ...
            if name == "":
                error_screen = Toplevel(root)
                Label(error_screen, text="Name cannot be empty. Please enter your name.").pack(side=TOP)
                Button(error_screen, text="OK", command=error_screen.destroy).pack(side=TOP)
            else:
                ...
                new_window.destroy()
                statistics()  # Show statistics after writing the score
    ...
    def buttons(clicked_answer):
        ...
        if clicked_answer == questions[enter][question]:
            ...
            Label(answer_window, text=f"INCORRECT\n The answer was {questions[enter][question]}", fg="red").pack(side=TOP)
...
# Welcome screen and quiz selection functions remain unchanged
